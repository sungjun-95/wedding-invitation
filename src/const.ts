import dayjs from "dayjs"
import utc from "dayjs/plugin/utc"
import timezone from "dayjs/plugin/timezone"
import "dayjs/locale/ko"

dayjs.extend(utc)
dayjs.extend(timezone)
dayjs.locale("ko")

export { dayjs }

export const WEDDING_DATE = dayjs.tz("2026-07-11 12:10", "Asia/Seoul")
export const WEDDING_DATE_FORMAT = `2026년 07월 11일 오후 12시${WEDDING_DATE.minute() === 0 ? "" : " 10분"}`

// 예식 당월 휴무일. 켈린더에 표시하기 위함.
// 예: 예식일 8월 -> 8월 15일 광복절
export const HOLIDAYS = [17]

export const LOCATION = "신도림 라마다 호텔 5층 세인트 그레이스 홀"
export const LOCATION_ADDRESS = "서울 구로구 신도림동 427-3, 5층"

// 카카오톡 공유 시 위치 정보로 사용할 주소.
// LOCATION 과 동일하게 설정해도 무방하나, 필요에 따라 좀 더 상세히 작성 가능.
export const SHARE_ADDRESS = LOCATION
export const SHARE_ADDRESS_TITLE = LOCATION

// 네이버 지도 및 카카오 네비게이션에 사용할 좌표. [경도, 위도] 형식.
export const WEDDING_HALL_POSITION = [126.8857279, 37.5060663]

// 네이버 지도의 웨딩홀 장소 ID
// 네이버 지도 웹페이지에서 웨딩홀 검색 후 URL에서 확인 가능.
// 예: https://map.naver.com/p/entry/place/13321741 -> 13321741
export const NMAP_PLACE_ID = 986184099

// 카카오 지도의 웨딩홀 장소 ID
// 카카오 지도 웹페이지에서 웨딩홀 검색 후 해당 장소에서 상세보기 클릭 시 URL에서 확인 가능.
// 예: https://place.map.kakao.com/8634826 -> 8634826
export const KMAP_PLACE_ID = 1212235250

export const BRIDE_FULLNAME = "이하은"
export const BRIDE_FIRSTNAME = "하은"
export const BRIDE_TITLE = "장녀"
export const BRIDE_FATHER = "이호준"
export const BRIDE_MOTHER = "이선미"
export const BRIDE_INFO = [
  {
    relation: "신부",
    name: BRIDE_FULLNAME,
    phone: "010-8871-6822",
    account: "하나은행 227-910442-13307",
  },
  {
    relation: "신부 아버지",
    name: BRIDE_FATHER,
    phone: "010-3265-6747",
    account: "하나은행 069-18-29374-2",
  },
  {
    relation: "신부 어머니",
    name: BRIDE_MOTHER,
    phone: "010-5667-6822",
    account: "신한은행 110-297-825859",
  },
]

export const GROOM_FULLNAME = "장성준"
export const GROOM_FIRSTNAME = "성준"
export const GROOM_TITLE = "장남"
export const GROOM_FATHER = "장지영"
export const GROOM_MOTHER = "박혜숙"
export const GROOM_INFO = [
  {
    relation: "신랑",
    name: GROOM_FULLNAME,
    phone: "010-3323-2250",
    account: "새마을금고 9003-3026-9507-7",
  },
  {
    relation: "신랑 아버지",
    name: GROOM_FATHER,
    phone: "010-4713-2250",
    account: "신한은행 110-003-609361",
  },
  {
    relation: "신랑 어머니",
    name: GROOM_MOTHER,
    phone: "010-6216-2250",
    account: "우리은행 1002-850-289612",
  },
]

